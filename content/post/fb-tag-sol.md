...
class Solution {
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        Map<Integer, LinkedHashMap<Integer, PriorityQueue<Integer>>> map = new LinkedHashMap<>();
        dfs(root, 0, 0, map);
        List<List<Integer>> list = new ArrayList<>();
        for (LinkedHashMap<Integer, PriorityQueue<Integer>> ys : map.values()) {
    ...
    private void dfs(TreeNode root, int x, int y, Map<Integer, LinkedHashMap<Integer, PriorityQueue<Integer>>> map) {
        if (root == null) {
            return;
        }
        if (!map.containsKey(x)) {
            map.put(x, new LinkedHashMap<>());
        }
